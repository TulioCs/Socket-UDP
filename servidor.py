...
def main():
	while True:
		...
		cliente, cliente_end = servidor.recvfrom(buffer_size)

		client_list[cliente_end] = cliente
		print(client_list)

		for cliente_endereco in client_list: 

			if(client_list[cliente_endereco] == "r"):
				#Enviando menu de escolha para o cliente
				servidor.sendto("\nA. Média\nB. Mediana\nC. Moda\nX. Para Sair\n\nOpção: ", cliente_endereco)

				#Obtendo o dado "opção" do cliente
				dado, new_cliente_endereco = servidor.recvfrom(buffer_size)

				if(new_cliente_endereco in client_list):
					cliente_endereco = new_cliente_endereco
